...
- Backend maps Gamification entities to the current SQL script tables; no separate `gam` schema is created for MVP.
- `ActivityLog` is strictly insert-only — no update/delete SQL generated by ORM.

## Assumptions

- Target database is SQL Server. Backend maps to current DB script tables (`Badge`, `StudentBadge`, `StudyStreak`, `TargetScore`, `ActivityLog`) instead of schema-prefixed tables.
