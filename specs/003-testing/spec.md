...
- **BR-12**: Timed-test countdown is synchronized with the server — client-side timer is display only; server enforces the actual deadline. An unlimited Practice Test (`DurationMinutes = 0`) returns elapsed time instead of a deadline.
- **BR-13**: When a timed test reaches 00:00, system automatically locks the interface, saves all selected answers at that point, and triggers force-submit workflow. Unlimited Practice Tests never enter this timeout flow.
...
- **BR-15a Test access authorization**: discovery is not an authorization boundary. StartSession must reject a personal Test unless `GeneratedForStudentID` equals the authenticated Student. A shared Test is startable only when it is an `Active` BlueprintExam with null `GeneratedForStudentID`, an `Active` Blueprint, and `Blueprint.Grade = Student.CurrentGrade`.
- **BR-15b Archive race**: StartSession and shared-Test archive serialize on the Test row. A session that acquires the lock before archive may be created and finish normally; after archive commits, no new session may start.
- **BR-15c Resume contract**: duplicate StartSession returns `409 TESTING_SESSION_ALREADY_IN_PROGRESS` with `existingSessionId`. Session content returns every persisted answer, selected option, composite part answer, `HasTimeLimit`, nullable `remainingSeconds`, and `elapsedSeconds` so a reload cannot display a blank state while hidden answers remain in the database.
- **BR-15d Timeout authority**: the browser timer is display-only. Timed tests verify ownership, `InProgress` status, and `StartTime + DurationMinutes <= server UTC now` before invoking force-submit with `SubmissionType = TimeoutSubmit`. Calling it early returns `409 TESTING_SESSION_NOT_EXPIRED`. Auto-save after the deadline returns `409 TESTING_SESSION_EXPIRED`, and a normal submit received after the deadline is converted server-side to `TimeoutSubmit`. Calling timeout-submit for an unlimited Test returns `409 TESTING_TEST_HAS_NO_TIME_LIMIT`.
