...
- **Mistral OCR** integration (UC-39): an Expert posts exactly one complete question image as multipart field `file` to `POST /api/question-bank/questions/ocr-draft`. The backend applies the same JPEG/PNG/WebP 5 MB magic-byte validation, rate limits by authenticated Expert (10/minute, no queue), and returns an unpersisted draft plus up to three detected image candidates. The draft may expose normalized observed marks (`None`, `Circled`, `Ticked`, `Crossed`, `Highlighted`, `Unknown`) but always clears correctness suggestions. The Expert may choose one candidate or the original source image; selection is uploaded through the existing Cloudinary endpoint only when applying the draft. Mistral credentials and raw provider failures stay server-side; observed marks, answers, and image candidates are never authoritative.
