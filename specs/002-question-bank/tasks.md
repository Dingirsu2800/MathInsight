...
- [ ] Create EF `IEntityTypeConfiguration` for all 7 entities mapped to current DB script tables:
  - [ ] `QuestionConfiguration` — composite index `(status, grade)`, FK to `experts`, `tag_difficulties`; map API question type enums to DB values (`SingleChoice`, `MultipleChoice`, `TrueFalse`, `ShortAnswer`)
...
- [ ] Schema alignment check: verify SQL script allows `Question.Status = Pending` before implementing Admin re-review flow; verify `TagDifficulty.LevelValue` has a unique constraint.
...
- [ ] Integration tests (xUnit):
  ...
  - [ ] UC-21: API enum `TRUE_FALSE` persists as DB value `TrueFalse`; `MULTIPLE_SELECT` persists as `MultipleChoice`
